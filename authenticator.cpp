...
int main() {
    ...
    // Step 3: checksum to k_iValidCodeCheckdum and display message
    if (checksum == k_iValidCodeChecksum) {
        std::cout << "Message Code Checksum is valid: " << k_iValidCodeChecksum << std::endl;
    } else {
        std::cout << "Message Code Checksum is invalid." << std::endl;
    }

    // Step 5: Report which pairs don’t match, use != in branch condition
    if (input1 != k_cAuthenticatorCodeCharacter1) {
        std::cout << "First characters do not match: Message " << input1 << "; Authenticator " << k_cAuthenticatorCodeCharacter1 << std::endl;
    }
    if (input2 != k_cAuthenticatorCodeCharacter2) {
        std::cout << "Second characters do not match: Message " << input2 << "; Authenticator " << k_cAuthenticatorCodeCharacter2 << std::endl;
    }
    if (input3 != k_cAuthenticatorCodeCharacter3) {
        std::cout << "Third characters do not match: Message " << input3 << "; Authenticator " << k_cAuthenticatorCodeCharacter3 << std::endl;
    }

     // Step 6: Compare if message code and Authenticator strings do not match using > and <
    if (messageCode < authenticatorCode) {
        std::cout << "Message code (CKB) is lexicographically less than the Authenticator code (ECA)." << std::endl;
    } else if (messageCode > authenticatorCode) {
        std::cout << "Message code (NSA) is lexicographically greater than the Authenticator code (ECA)." << std::endl;
    }
