...
void Game::initializeGrid()
{
	...
	for (int iRow = 0; iRow < this->m_nGridRows; ++iRow)
	{
		...
		{
			a_cGrid[iRow][iColumn] = '~';/*?? assign '~' to element of 2-D array a_cGrid indexed by iRow and iColumn ?? */
		}
	}
	return;
}
...
std::tuple<int, int>  Game::fireTorpedo()
{
	...
	this->m_nTorpedoes -= 1;/* ?? decrement this->m_nTorpedoes ?? */
	...
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	this->a_cGrid[iRow][iColumn] = '*';/* ?? Assign '*' to value of a_cGrid at the row and column index returned from promptForGridCoord(). ??*/
...
void Game::placeBoatRandomly()
{
	...
	this->a_cGrid[this->m_iRowBoat][this->m_iColumnBoat] = 'V';/* ?? Assign 'V' to the element of 2-D member variable a_cGrid. ?? */
...
void Game::placeBoat()
{
	...
	this->m_iColumnBoat = iColumn;/* ?? Assign value to this->m_iRowColumn ?? */
	return;
}
...
bool Game::isCloseToTarget(int iRow, int iColumn)
{
	...
	// the target boat. 
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	if (std::abs(this->m_iRowBoat - iRow) <= 1 && std::abs(this->m_iColumnBoat - iColumn) <= 1)
	{
		bClose = true;
	}
...
void Game::play()
{
	...
	while ((!bSunk) && (this->m_nTorpedoes > 0))
	{
		...
		if (bSunk)
		{
			std::cout << "Magazine: " << "Magazine: " << this->m_nTorpedoes << " torpedoes" << std::endl;
		}
		...
		if (this->m_nTorpedoes == 0 && !bSunk) 
		{
			...
			this->displayGrid(true);/* ?? call member function to dislay the grid and show the location of the battle boat ?? */
